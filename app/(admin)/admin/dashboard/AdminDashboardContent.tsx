...
export default function AdminDashboardContent() {
  ...
  async function handleAssign(leadId: string, agentId: string) {
    ...
    if (res.ok) {
      // Update the specific lead with the server response to ensure consistency
          ...
          prev.map((l) =>
            l._id === leadId
              ? {
                  ...
                  assignedTo: {
                    _id: agentId,
                    name: agent?.name ?? data.lead.assignedTo?.name ?? "",
                    email: agent?.email ?? data.lead.assignedTo?.email ?? "",
                  },
                }
              : l
          )
